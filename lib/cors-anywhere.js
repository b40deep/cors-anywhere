...
function parseURL(req_url) {
  ...
  //                                              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //                                            2:host
  console.log(`_____________________parse_match ${match[0]},  ${match[1]}, ${match[2]},  ${match[3]}, ${match[4]},   ${match[5]},`);
  if (!match) {
    return null;
  }
  if (!match[1]) {
  ...
  var match2 = req_url.match(/^(https?:\/\/)?(([^\/?]+?)(?::(\d{0,5})(?=[\/?]|$))?)([\/?][\S\s]*|$)/i);
  console.log(`_____________________parse_match ${match2[0]},  ${match2[1]}, ${match2[2]},  ${match2[3]}, ${match2[4]},   ${match2[5]},`);
  var parsed = url.parse(req_url);
  if (!parsed.hostname) {
    // "http://:1/" and "http:/notenoughslashes" could end up here.
    return null;
  }
  console.log(`_____________________parse_AFTER ${req_url}`);
  return parsed;
}
