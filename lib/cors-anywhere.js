...
function parseURL(req_url) {
  console.log(`_____________________parse_BEFORE ${req_url}`);
  // var match = req_url.match(/^(?:(https?:)?\/\/)?(([^\/?]+?)(?::(\d{0,5})(?=[\/?]|$))?)([\/?][\S\s]*|$)/i);
  //                              ^^^^^^^          ^^^^^^^^      ^^^^^^^                ^^^^^^^^^^^^
  //                            1:protocol       3:hostname     4:port                 5:path + query string
  //                                              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //                                            2:host
  // console.log(`_____________________parse_match   1_${match[1]}, 2_${match[2]},  3_${match[3]}, 4_${match[4]}, URL_${req_url} ,`);
  //boydee handling the single slash bug
  var match2 = req_url.match(/^(https?:\/)(.*)/);
  console.log(`_____________________parse_match2   {match2[0]}, 1_${match2[1]}, 2_${match2[2]},  {match2[3]}, {match2[4]},  ,`);
  
  var parsed = url.parse('https://'+match2[2],true);
  
  //Boydee commented out all these and just reshaped the url with https://blabla to fix the single / on https:/blabla
  /*
  if (!match) {
    return null;
  }
  if (!match[1]) {
    ...
    // Scheme is omitted.
    console.log(`_____________________parse_lastIndex of, === -1? ${req_url.lastIndexOf('//', 0)}`);
  ...
  */
  console.log(`_____________________parse_AFTER ${parsed}`);
  console.log(`_____________________parse_AFTER2 ${JSON.stringify(parsed)}`);
  return parsed;
}
